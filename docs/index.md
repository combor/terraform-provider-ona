---
# generated by https://github.com/hashicorp/terraform-plugin-docs
page_title: "ona Provider"
description: |-
  Terraform provider for managing Gitpod https://gitpod.io resources on ona.com https://ona.com.
---

# ona Provider

Terraform provider for managing [Gitpod](https://gitpod.io) resources on [ona.com](https://ona.com).

## Example Usage

```terraform
provider "ona" {
  api_key  = var.ona_api_key  # or set GITPOD_API_KEY env var
  base_url = var.ona_base_url # optional, defaults to https://app.ona.com/api
}
```

<!-- schema generated by tfplugindocs -->
## Schema

### Optional

- `api_key` (String, Sensitive) API key. Falls back to `GITPOD_API_KEY` env var.
- `base_url` (String) API base URL. Falls back to `GITPOD_BASE_URL` env var. Defaults to `https://app.ona.com/api`.
- `max_retries` (Number) Maximum number of retries per request. Defaults to `2`. Set to `0` to disable retries.
- `request_timeout` (String) Per-attempt request timeout as a Go duration (for example `20s` or `2m`). If unset, requests have no SDK-level timeout.
